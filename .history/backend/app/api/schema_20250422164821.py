...
class ParameterSpaceResponse(BaseModel):
    ...
    status: TaskStatusResponse = Field(..., description="Status of the task")
...
class TaskDetails(BaseModel):
    ...
    updated_at: datetime = Field(..., description="Last update time")
    progress: float = Field(..., ge=0, le=100, description="Progress percentage")
    parameter_space: Dict[str, Any] = Field(..., description="Parameter space configuration")
    strategy: Optional[Dict[str, Any]] = Field(None, description="Optimization strategy")
    initial_designs: Optional[List[Dict[str, Any]]] = Field(None, description="Initial design points")
    results: Optional[List[Dict[str, Any]]] = Field(None, description="Submitted results")
    next_points: Optional[List[Dict[str, Any]]] = Field(None, description="Next recommended points")
    ...
    def validate_task_consistency(self):
        """Validate task data consistency based on status."""
        if self.status in ["running", "completed", "paused"] and not self.strategy:
            raise ValueError("Tasks in 'running', 'completed', or 'paused' status must have a strategy")
        
        if self.status in ["completed"] and not self.results:
            raise ValueError("Completed tasks must have results")
