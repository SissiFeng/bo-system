...
def get_or_create_optimizer(task_id: str) -> BayesianOptimizer:
            ...
            for res in results_data:
                ...
                if isinstance(res, dict) and 'parameters' in res and 'objectives' in res:
                    try:
                        ...
                        internal_point = parameter_space_obj.point_to_internal(res['parameters'])
    ...
    try:
        optimizer = BayesianOptimizer(
            parameter_space=parameter_space_obj,
    ...
    try:
        ...
        for obj in objective_names:
            if directions[obj] > 0:  # maximize
                ...
                nadir_point[obj] = min(point['objectives'][obj] for point in points)
            else:  # minimize
        ...
        logger.error(f"Computing Pareto front failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to compute Pareto front: {str(e)}")


@router.get("/uncertainty/{task_id}", response_model=schema.UncertaintyAnalysis)
async def get_uncertainty_analysis(
    task_id: str = Path(..., description="Task ID"),
    parameter_name: Optional[str] = Query(None, description="Parameter to analyze uncertainty for")
):
    """
    Get uncertainty analysis for a specific parameter or all parameters.
    """
    if task_id not in tasks:
        raise HTTPException(status_code=404, detail=f"Task {task_id} not found")
    ...
    try:
        ...
        if param_info["type"] == "continuous":
            ...
            # 创建20个均匀分布的点
            param_values = np.linspace(param_min, param_max, 20).tolist()
        elif param_info["type"] == "discrete" or param_info["type"] == "integer":
            param_min = param_info["min"]
            param_max = param_info["max"]
            # 不超过20个点
            step = max(1, (param_max - param_min) // 19)
            param_values = list(range(param_min, param_max + 1, step))
        ...
        for param in param_names:
            ...
            if param_type == "categorical":
                ...
                # 使用众数作为固定值
                fixed_params[param] = Counter(observed_values).most_common(1)[0][0]
            else:
                # 使用中值作为固定值
