...
class TestDesignGenerator(unittest.TestCase):
    ...
    def test_lhs_generator_with_constraints(self):
        ...
        # It may not be able to generate all points, but at least some should be generated
        self.assertTrue(len(designs) > 0, f"Expected > 0 designs, but got {len(designs)}")
        self.assertTrue(len(designs) <= n_designs)

        # Verify that the generated design points satisfy the constraints
        for design in designs:
            # Manually check constraints, because check_constraints may require design to contain id
            self.assertTrue(constrained_param_space.check_constraints(design))
            self.assertTrue(design["x1"] + design["x2"] <= 8)

    # TODO: Add tests for other DesignGenerator methods
    # For example, generate_random_designs, generate_grid_designs, etc.
