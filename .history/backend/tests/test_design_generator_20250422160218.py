...
# 假设 DesignGenerator 类在 bo_engine.design_generator 中
from bo_engine.design_generator import DesignGenerator, DesignType, LatinHypercubeDesignGenerator
...
class TestDesignGenerator(unittest.TestCase):
    ...
    def test_lhs_generator_init(self):
        """测试拉丁超立方生成器的初始化"""
        # 注意：DesignGenerator 的初始化现在需要 output_dir
        generator = DesignGenerator(self.param_space, seed=42, output_dir=self.output_dir)
        # 检查子类初始化是否正确
        lhs_gen = LatinHypercubeDesignGenerator(self.param_space, seed=42) # 假设子类可以独立初始化用于测试
        self.assertIsInstance(lhs_gen, DesignGenerator) # 检查是否继承基类（虽然基类不是直接被实例化的）
        self.assertEqual(lhs_gen.parameter_space, self.param_space)
        # self.assertEqual(lhs_gen.seed, 42) # 子类现在没有 seed 属性了

    def test_lhs_generator_generate(self):
        """测试拉丁超立方生成器生成设计点"""
        n_designs = 10
        # DesignGenerator 现在管理所有方法
